...
<div class="container-fluid py-4">
    <div class="row mb-3 align-items-center">
        <div class="col-md-8">
            <h3 class="fw-bold text-primary mb-1">
                <i class="fas fa-file-invoice me-2"></i>New Warranty Sale Invoice
            </h3>
            <p class="text-muted small mb-0">Generate printable warranty invoice for sub-customers without affecting inventory stock or ledgers.</p>
        </div>
    ...
    <form action="{{ route('dummy-invoices.store') }}" method="POST" id="dummyInvoiceForm">
        @csrf
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-3">
                        ...
                        <input type="text" name="invoice_no" class="form-control fw-bold" value="{{ old('invoice_no', $nextInvoiceNo) }}" required>
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="text" name="gate_pass_no" class="form-control" placeholder="e.g. 25911-26" value="{{ old('gate_pass_no') }}">
                    </div>

                    <div class="col-md-6">
                        <label class="form-label fw-bold">Customer Name <span class="text-danger">*</span></label>
                        <input type="text" name="customer_name" class="form-control fw-bold" placeholder="e.g. M/S HAMZA Medicine Company or Ali / Ahmed / Shabir" value="{{ old('customer_name') }}" required>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label fw-bold">Licence No.</label>
                        <input type="text" name="licence_no" class="form-control" placeholder="e.g. 04-361-0171-110834D" value="{{ old('licence_no') }}">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label fw-bold">Licence Expiry</label>
                        <input type="text" name="licence_expiry" class="form-control" placeholder="e.g. 02.11.2028" value="{{ old('licence_expiry') }}">
                    </div>

                    <div class="col-md-12">
                        <label class="form-label fw-bold">Customer Address</label>
                        <textarea name="customer_address" class="form-control" rows="2" placeholder="e.g. House No. 15-16 Near Muhammad Complex, Shamsabad Colony, Multan.">{{ old('customer_address') }}</textarea>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-8">
                        ...
                        <textarea name="warranty_text" class="form-control small" rows="3">{{ old('warranty_text', $defaultWarranty) }}</textarea>
                    </div>
                    <div class="col-md-4">
                        <div class="mb-2">
                            <label class="form-label fw-bold">Signatory Name</label>
                            <input type="text" name="signatory_name" class="form-control" value="{{ old('signatory_name', 'SEEMA KHAN') }}">
                        </div>
                        <div>
                            <label class="form-label fw-bold">Signatory Title</label>
                            <input type="text" name="signatory_title" class="form-control" value="{{ old('signatory_title', 'Production Incharge') }}">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="text-end mb-5">
            <button type="submit" name="save_only" class="btn btn-primary px-4 py-2 fw-bold shadow-sm me-2">
                <i class="fas fa-save me-2"></i> Save Invoice
            </button>
            <button type="submit" name="print_after_save" value="1" class="btn btn-success px-4 py-2 fw-bold shadow-sm">
                <i class="fas fa-print me-2"></i> Save & Print Invoice
            </button>
        </div>
    </form>
</div>

<!-- System Products JSON for JS autocomplete -->
<script>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function addRow(data = {}) {
        ...
        tr.innerHTML = `
            ...
            <td>
                ...
                <input type="text" name="items[${rowIndex}][product_name]" class="form-control form-control-sm fw-bold product-name-input" placeholder="Type / Search Product..." list="productList_${rowIndex}" value="${data.name || data.product_name || ''}" oninput="onProductNameInput(this, ${rowIndex})" autocomplete="off" required>
            ...
            <td><input type="text" name="items[${rowIndex}][batch_no]" class="form-control form-control-sm text-center fw-bold text-primary" placeholder="Batch #" value="${data.batch || data.batch_no || ''}"></td>
            <td><input type="text" name="items[${rowIndex}][mfg_date]" class="form-control form-control-sm text-center" placeholder="MM-YY" value="${data.mfg || data.mfg_date || ''}"></td>
            <td><input type="text" name="items[${rowIndex}][exp_date]" class="form-control form-control-sm text-center" placeholder="MM-YY" value="${data.exp || data.exp_date || ''}"></td>
            ...
            <td><input type="number" step="any" name="items[${rowIndex}][total_amount]" class="form-control form-control-sm text-end total-input fw-bold text-success" placeholder="0.00" value="${data.total || data.total_amount || 0}" readonly></td>
    ...
    // Start with 1 clean empty row with placeholders
    addRow();
});
</script>
@endsection
